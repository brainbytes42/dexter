...
public class MainView {
    ...
    @FXML private Button pickUnlabeledButton;
    @FXML private SplitMenuButton checkLabelButton;
    @FXML private MenuItem clearCheckLabelHistoryMenuItem;
    @FXML private FlowPane classFilterPane;
    @FXML private MenuBar menuBar;
    @FXML private BorderPane statusBar;
    @FXML private TextField filterTextField;
    @FXML private SplitMenuButton addDataSourcesSplitMenuButton;
    @FXML private Button removeDataSourcesButton;
    @FXML private InteractiveScatterChart<Double, Double> scatterChart;
    @FXML private TreeView<DataSource> dataSourceTree;
    @FXML private ToggleButton enableDimReductionButton;
    ...
    private void initialize() {
        ...
        this.domainAdapter.getDataSourceActions().getDefaultAddAction().subscribe(defaultActionOpt -> {
            ...
            if (defaultActionOpt.isPresent()) {
                ...
                defaultActionOpt.get()
                                .getDescription()
                                .toObservable()
                                .subscribe(descr -> this.addDataSourcesSplitMenuButton.setTooltip(new Tooltip(descr)));
        ...
        Filter
         */
        AtomicReference<Map<Optional<ClassLabel>, Boolean>> classLabelEnabledFilterModel = new AtomicReference<>(Collections.emptyMap());
        ...
        this.domainAdapter.getClassLabels()
                          .doOnNext(classLabels -> {
                              classLabelEnabledFilterModel.updateAndGet(old -> {
                                  ...
                                  classLabels.forEach(label -> update.putIfAbsent(label, true)); // enabled by default
                                  return update;
                              });
                          ...
                          .skip(1)
                          .subscribe(filterUpdate -> this.domainAdapter.replaceDataFilter(filterUpdate.get(0), filterUpdate.get(1)));
        ...
        this.pickUnlabeledButton.disableProperty()
                                .bind(JavaFxObserver.toBinding(
                                        this.dexterModel.getActiveLearning().getClassificationUncertainty().map(Collection::isEmpty)));
        ...
        this.checkLabelButton.disableProperty()
                             .bind(JavaFxObserver.toBinding(
                                     this.dexterModel.getActiveLearning().getExistingLabelsUncertainty().map(Collection::isEmpty)));
        ...
        JavaFxObservable.actionEventsOf(this.evaluateModelButton)
                        .subscribe(actionEvent -> ModelQualityView.createAndShow(this.dexterModel.getClassificationModel()
    ...
    public void checkLabel(ActionEvent actionEvent) {
        this.domainAdapter.getFilteredDomainData().firstElement().subscribe(entities -> {
            ...
            labeledNotYetCheckedEntities.removeAll(this.appContext.getInspectionHistory().getLabeledEntities().getValue());
            ...
            if (!labeledNotYetCheckedEntities.isEmpty()) {
                ...
                ActiveLearning activeLearning = new ActiveLearning(RandomForest::new, this.domainAdapter.getClassLabels().blockingFirst());
                Evaluation evaluation = activeLearning.evaluateClassification(featureSpace, labeledNotYetCheckedEntities);
                ...
                Table<DomainDataEntity, ClassLabel, Double> probabilities = activeLearning.classificationProbabilities(featureSpace,
                ...
                probabilitiesSortedByDoubt.forEach(
                        entry -> System.out.printf("Entry (is %s): %s  -->  %f %n", entry.getKey().getClassLabel().getValue(), entry,
                                                   entry.getKey()
                                                        .getClassLabel()
                                                        .getValue()
    ...
    private void initProgressMonitor() {
        ...
        this.appContext.getTaskMonitor().getCurrentTasks().toObservable().map(tasks -> {
            ...
            tasks.stream().map(task -> {
                ...
                taskUpdates.add(Observable.combineLatest(task.getName().toObservable(), task.getProgress().toObservable(),
    ...
    private void initializeScatterChart() {
        ...
        Observable<ObservableList<Series<Double, Double>>> chartDataObs = this.dexterModel.getLowDimData()
    ...
    private void displayAddActionResultMessage(DataSourceActions.AddAction.Result result) {
        switch (result.getState()) {
            ...
            case ADDING_FAILED:
                new Alert(AlertType.WARNING, "DataSource could not be added (" + result.getState() + "): " + result.getMessage(),
                          ButtonType.OK).showAndWait();
                break;
        }
    }
